...
friendSchema.statics.upsertId = function (id, callback) {
    return this.update(
        {"id": id},
        {"$set": {
            "id": id,
...
friendSchema.statics.upsertUser = function (user, callback) {
    return this.update(
        {"id": user.id},
        {"$set": user},
        {upsert: true},
...
friendSchema.statics.removeStale = function (ids, callback) {
    return this.remove({"id": {"$nin": ids}}, callback);
};

friendSchema.statics.list = function (callback) {
    return this.find({}, callback);
};

friendSchema.statics.queryIds = function (callback) {
    return twitterClient.get('friends/ids', callback);
};

friendSchema.statics.queryList = function (cursor, callback) {
    var self = this;
    cursor = (cursor === undefined) ? -1 : cursor;
    return twitterClient.get('friends/list', {'cursor': cursor}, callback);
};

friendSchema.statics.cacheIds = function () {
    var self = this;
...
friendSchema.statics.cacheList = function () {
    var self = this;
...
module.exports = mongoose.model('Friend', friendSchema, config.twitter.username + 'friends');